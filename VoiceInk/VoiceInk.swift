...
/// `Services/Updater.swift`). Manual "Check for Updates…" only — automatic
/// background update checking was removed.
@MainActor
class UpdaterViewModel: ObservableObject {
    private let updater = Updater()

    /// Guards against concurrent install triggers (the update prompt's Install and a
    /// second manual check could both fire `startInstall`). @MainActor-isolated, so a
    /// plain Bool is safe.
